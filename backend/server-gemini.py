...
client = genai.Client(api_key="AQ.Ab8RN6Koh3kgGvL1keX6QxRD0J0aEBbzKROljCCEloJS5QSuFw") # replace with your Gemini API key

app = Flask(__name__)


EPISODE_SIZE = 8 # number of conversation turns before generating episodic memory
conversations = {}

# function to generate LLM response using Gemini API
def llm_generate(prompt, max_tokens=120, temp=0.3):
...
# functions to load and save NPC data
...
# function to score how relevant a memory is to the player's current message
...
# function to convert recent conversation into long-term memory entries using LLM
...
# function to append new memories to the NPC's memory file, avoiding duplicates
...
# function to decay importance of memories over time
def decay_memories(memories, decay=0.1):
    for memory in memories:
        memory["importance"] = max(memory.get("importance", 5) - decay, 1)
    return memories

# function to calculate trust level based on conversation length (simple heuristic)
...
# function to determine which secrets the NPC can share based on trust level
...
# main API endpoint for player messages
